...
# ------------------------------------Swin Transformer V2 End--------------------------------#

# # ------------------------------------C3_KAN Start--------------------------------#
# class C3_KAN(nn.Module):
#     # CSP Bottleneck with 3 KAN convolutions
...
#         c__ = int(c_ * e)
#         self.kanconv = KAN_Convolutional_Layer(n_convs=1, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), device='cpu')
#         self.cv = Conv(c_, c__, 1, 1)
#         self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
#
#     def forward(self, x):
#         x1 = self.kanconv(x)
...
#         y2 = self.kanconv(x1)
#         y2 = self.cv(y2)
#         return self.kanconv(torch.cat((y1, y2), dim=1))
