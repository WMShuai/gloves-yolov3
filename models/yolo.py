...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv, SELayer, conv_bn_hswish, SwinV2_CSPB,
                BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, CBAM, GhostModuleV2, h_swish, h_sigmoid, MobileNet_Block}:
            c1, c2 = ch[f], args[0]
        ...
        # TODO: channel, gw, gd
            ...
            c2 = ch[f] // args[0] ** 2
        elif m in [MobileOne, MobileOneBlock]:
            c1, c2 = ch[f], args[0]
            c2 = make_divisible(c2 * gw, 8)
            args = [c1, c2, n, *args[1:]]
        else:
...
class ASFF_Detect(nn.Module):  # add ASFFV5 layer and Rfb
    ...
    def __init__(self, nc=20, anchors=(), ch=(), multiplier=0.5, rfb=False, inplace=True):  # detection layer
        super().__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
